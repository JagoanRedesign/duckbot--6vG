...
export async function setsticker(ctx) {
  let langs = await getLang(ctx)
  let c = await getPing(ctx)
  try {
    if (!ctx.message.reply_to_message) {
    ...
    if (ctx.message.reply_to_message.sticker?.is_animated) {
      let packName = `property_of_${String(process.env.USERNAME)
        .replace(/^\@/, '')
        .trim()}`
      let found: any = false
      let packNum = 0
      ...
      let packName = `sticker_by_${String(process.env.USERNAME).replace(/^\@/, '').trim()}`
      let found: any = false
      let packNum = 0
      while (true) {
        try {
